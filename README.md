...
- Gateway API: Envoy Gateway installed by default
...
| Envoy Gateway | `1.8.0` | `1.8.0` | `1.8.0` | `1.8.0` |
...
- Internet access to the Envoy Gateway OCI Helm chart when Envoy Gateway is enabled
...
| `./cns.sh install <stack-version> --set install_envoy_gateway=false` | `--set install_envoy_gateway=false` | Skips Envoy Gateway Helm deployment. |
| `./cns.sh install <stack-version> --set envoy_gateway_version=<version>` | `--set envoy_gateway_version=<version>` | Deploys the requested Envoy Gateway Helm chart version instead of the stack default. |
...
`--set` may be repeated and may override any top-level key defined in the selected stack file. The default install behavior uses the selected stack file's `install_gpu_operator`, `install_nfs_provisioner`, `install_metallb`, `install_envoy_gateway`, `cuda_driver_container_version`, `envoy_gateway_version`, and `metallb_load_balancer_ip_range` values. Install toggle values must be `true` or `false`. `--set cuda_driver_container_version=<version>` requires GPU Operator installation and cannot be combined with `--set install_gpu_operator=false`. `--set envoy_gateway_version=<version>` requires Envoy Gateway installation and cannot be combined with `--set install_envoy_gateway=false`.
...
By default, CNS installs GPU Operator with the selected stack file's CUDA driver container version. CNS also enables the node as an NFS server, exports `/srv/cns/nfs`, deploys `nfs-subdir-external-provisioner`, creates the default `nfs-client` StorageClass, installs MetalLB, configures a Layer 2 load-balancer address pool, and installs Envoy Gateway.
...
To override the stack default Envoy Gateway chart version:
...
Uninstall removes the Envoy Gateway release, MetalLB release, NFS provisioner release, and CNS export configuration, but preserves PVC data under `/srv/cns/nfs`.
...
CNS_TEST_PASSWORD='<target-password>' ./tests/test_cns_matrix.py \
  ...
  --set install_envoy_gateway=false \
  --fail-fast
```
...
Use `--set envoy_gateway_version=<version>` to validate a specific Envoy Gateway chart version. This override requires `install_envoy_gateway=true`.
...
The versions currently pinned in this repository are defined by the stack files:
...
- Envoy Gateway Helm chart: `1.8.0`
